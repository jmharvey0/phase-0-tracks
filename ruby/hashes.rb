...
designer_response[:picky] = gets.chomp.to_s 
	if designer_response[:picky] == "yes" || designer_response[:picky] == "y"
		designer_response[:picky] = true
	else
		designer_response[:picky] = false
	end 

p designer_response

puts "If you need to make an update, please enter the name of the key you wish to update. If you don't require any updates, say 'none'."
		key_change = gets.chomp.to_s
			if key_change == "none"
				exit
			elsif key_change == "name"
